...
namespace network
{
    ...
    void handleBoilertimer()
    {
        ...
        if (webServer.method() == HTTP_POST)
        { //  POST
            if (webServer.hasArg("timerValue"))
            {
                settings::boilerDelay = webServer.arg("timerValue").toInt();
            }
            settings::SaveSettings();
        }
        ...
        String htmlString, valueList = "";

        File f = LittleFS.open("/boilertimer.html", "r");

        for (size_t i = 10; i < 61; i += 10)
        {
            valueList += "<option";
            if (settings::boilerDelay == i)
                valueList += " selected";
            valueList += " value=\"";
            valueList += (String)i;
            valueList += "\">";
            valueList += (String)i;
            valueList += " minutes</option>";
            valueList += "\n";
        }
        ...
        htmlString.replace("%year%", (String)year(localTime));
        htmlString.replace("%delaylist%", valueList);
    ...
    void InitWifiWebServer()
    {
        ...
        webServer.on("/boilertimer.html", handleBoilertimer);
